...
- `ChatMessageCell.java` was **un-pinned** (kept at the Forkgram/upstream
  ...
  **Exception ported 2026-06-05:** the *forwarded-message date* (`751f51692`,
  "date of forwarded message in title") was nonetheless absent. Restored, but
  **Classic-specific:** the date renders on its **own 3rd line** under the forward
  name — modern appends it to the "Forwarded from" label (`.append(timeOfFwdMsg)`),
  which truncates on narrow/media bubbles (forward width ≈ media width). Impl: a
  separate `forwardedDateLayout` drawn one line below at the same per-line step,
  with reserved height (`namesOffset` ×2→×3 and the forward term in
  `replyStartY`/`summaryStartY`); lazy fetch via `ensureForwardDateTime` /
  `ForkApi.TLRPCMessages` when `fwd_from.date == 0`.
- "Fixed build" commits (`b42e168e3`, `b0a141c81`) are omitted — they're not
  features.
- **Classic nests `DialogsActivity` inside `MainTabsActivity`** (the bottom-tabs host). Only
  `MainTabsActivity` is in `actionBarLayout`'s fragment stack, so anything pushed to that stack
  does **not** reach the dialogs `ActionBar` that draws the visible title. Bit us **2026-06-05**:
  the connection-state title overlay ("Connecting"/"Updating"/"Connecting to proxy", set by
  `LaunchActivity.updateCurrentConnectionState` → `actionBarLayout.setTitleOverlayText`) went to the
  invisible `MainTabsActivity` while the dialogs title stayed stuck on the custom "Forkgram Classic".
  Fixed by also forwarding the overlay to `((MainTabsActivity) actionBarLayout.getLastFragment())
  .getDialogsActivity().getActionBar()`. **General rule:** anything aimed at the dialogs `ActionBar`
  (overlays, title tweaks) must be forwarded explicitly into the nested `DialogsActivity`.
- **Status-bar icon colour resets on resume.** `onCreate` sets a baseline decor `systemUiVisibility`
  of `LAYOUT_STABLE|LAYOUT_FULLSCREEN|LAYOUT_HIDE_NAVIGATION` (`0x700`, no `LIGHT_STATUS_BAR`). On
  resume the decor view reverts to that baseline *after* the resume's `checkSystemBarColors` set the
  light flag, so on a white (light-theme) dialogs list the status-bar icons go white-on-white
  (invisible); cold start is fine, and a chat round-trip fixed it (re-applied post-transition). Bit
  us **2026-06-05**. Fixed by re-applying in `LaunchActivity.onWindowFocusChanged(hasFocus)` →
  `checkSystemBarColors(false, true, false)` (fires after the revert).
- **First-login side-menu header glitches (`DrawerProfileCell` / `DrawerLayoutAdapter`).** The
  hand-ported drawer is built once at startup, before any account is activated, and was only refreshed
  on account switch — so a fresh first login showed three header bugs (all **2026-06-05**):
  - *Empty menu items* — `DrawerLayoutAdapter.resetItems()` early-returns when the selected account
    isn't `isClientActivated()`. Fixed by rebuilding the adapter every time the drawer opens
    (`DrawerLayoutContainer.openDrawer()` → `notifyDataSetChanged()`), which is timing-independent.
  - *White avatar for a photo-less account on the light theme* — `setUser` overrode the avatar colour
    with `key_avatar_backgroundInProfileBlue`, whose default is `0xffffffff` (white) and which the light
    theme doesn't redefine (dark theme does → it showed there). Fixed by dropping the override so
    `new AvatarDrawable(user)` keeps its always-visible per-user gradient.
  - *Name/phone drawn shifted down* — on the cold-login first layout the text height isn't measured
    yet, so `SimpleTextView`'s centred vertical offset draws the text low (clipping into the header
    edge); the pinned header isn't re-laid-out until an app restart. View bounds were always correct
    (`top=253`) — the shift is purely the draw offset. Fixed by forcing one more layout pass after
    `setUser` (post `requestLayout()` on name/phone/cell), which recomputes the offset.
- **`updateCheckIntervalRow` removed** from `ForkSettingsActivity` (set to `-1`, not allocated in
  `rowCount`) — F-Droid is the only update channel, so an in-app update-check interval is irrelevant
  (**2026-06-05**).
