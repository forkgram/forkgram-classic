...
## Modern 12.10.4 bump — no new redesign, the native build moved to `prebuild/` (`/classic-bump`, 2026-09-17)

Base `[F-Droid] 12.10.3.` (`d37ded1ba`) → `[Release] 12.10.4.` (`1f77ee0e7`, tag `12.10.4.0`, also
the modern `dev` tip). Upstream snapshot moved **12.10.1 (7038) → 12.10.3 (7089)**. Version
`12.10.7` / code `7091` = base 7090 + 1 — the *name* does not follow the base here, because classic
had already shipped 12.10.4/5/6 as issue releases on the old base; it continues classic's own
sequence.

**Measured, not hunted — there is no new chrome.** The per-pinned-file upstream delta between the
two snapshots is essentially nil: `ChatActivity` +4/−11, `ChatActivityEnterView` +1/−2,
`ProfileActivity` +2/−2, and `ActionBar` / `ActionBarLayout` / `BaseFragment` / `EmojiView` /
`ShareAlert` / `ChatAttachAlert` / `DialogCell` / the whole drawer **unchanged**. Only two new files
landed under `ui/**` (`AspectRatioFrameLayout`, `Gifts/SelectGiftsBottomSheet`), and just 6 genuinely
new fork commits arrived (subject-set diff — `dev` is force-pushed, so the raw 250-commit range
is meaningless).

### The three reshims

1. **`RLottieDrawable` lost its `String name` argument.** `(rawRes, name, w, h)` → `(rawRes, w, h)`
   and `(rawRes, name, w, h, startDecode, colorReplacement)` → `(rawRes, w, h, startDecode,
   colorReplacement)`. 13 call sites, all in pinned/classic-owned files (`Theme` ×7,
   `ProfileActivity` ×2, `ChatActivity`, `ChatActivityEnterView`, `DrawerProfileCell`,
   `LimitPreviewView`). **The dropped name was not a cache key** — the new constructor builds
   everything from `rawRes` + `w`/`h` + `colorReplacement` via `ResLottieMeta.find(rawRes)`, so
   `ProfileActivity`'s two `camera_outline` instances (dp(56) and dp(42)), which used to be
   distinguished by `String.valueOf(...)` vs `... + "_cell"`, stay independent. Verify that before
   dropping an argument: a shim that compiles can still be wrong.
2. **`AspectRatioFrameLayout` moved out of `com.google.android.exoplayer2.ui` into
   `org.telegram.ui`.** The pinned `ChatActivity` still imported the old package. The fix is to
   **delete** the import, not rewrite it — the class now sits in `ChatActivity`'s own package, which
   is exactly what the base file does (it has no import for it at all).
3. **`CaptionPhotoViewer` lost an import to the auto-merge.** `import android.graphics.Paint;` was
   silently dropped while merging the import block, breaking classic's own `backgroundBlur.getPaints(...)`
   blur path (that call does not exist in the base at all). A clean auto-merge can still delete a
   line the classic side needed — `Path` went with it, and was genuinely unused.

### Conflict resolutions (5 commits, 12 files)

- **Pinned, take classic** — `ChatActivity`, `ChatActivityEnterView`, `ProfileActivity`. All the
  conflicts are new fork features reaching into the pins: "hide sent media with spoiler"
  (`media_spoiler` + `MediaSpoiler`), "extract media from webpage preview" (which references the
  deleted `ChatReplyContainer`), "initial scroll to nearest dates in scheduled messages", the rich
  editor. They do not ship in classic — the standing pin tradeoff.
- **`FolderIcons.java` became `FolderIcons.kt`** — a modify/delete conflict with no text markers.
  Classic's **#81** icon-size fix (`dp(18)` → 24dp standalone / 20dp beside the title) was ported
  into the Kotlin file and the stale `.java` dropped. **New category:** a classic-patched file
  converted to Kotlin upstream.
- **`ForkSettingsActivity` — an ID collision.** 12.10.4 took `39` for `ID_LINK_REPLACEMENTS`, the
  number classic's **#118** had given `ID_SHOW_AI_SUMMARY`. Both are kept; classic's own ids now live
  in a private **900+** range so upstream's sequence can never land on them again. The ids are
  in-memory row tokens (`highlight(itemId)`), never persisted, so renumbering needs no migration.
  Also keep-both for the `ForkSettingsLock`/`DrawerItemsHelper` imports and the `#85` tagged constants.
- **`prebuild_fdroid.sh` auto-merged correctly this time** — classic's delta is down to the single
  `# [classic] #113:` comment tag, so the base's new `./prebuild/build_all.sh` entry point flowed in
  and its two now-stale seds (`build_boringssl.sh` API=16, the tde2e NDK bump) dropped themselves.
  Keeping a fix's classic delta to a *comment* is what made last bump's trap a no-op this time.
- **`strings.xml` ×4 (values, ru, es, pt-rBR)** — adjacent-insertion conflicts. Keep the base's new
  feature strings, keep classic's `AppName*`; the base rebranded its own to "Fork Client", which
  classic overrides to "Forkgram Classic".
- **`.github/workflows/tandroid.yml`** — modify/delete; the deletion stands.

### Native — a full cold rebuild, and a new host dependency

`jni/**` is 8216 files / −2125787 lines: the vendored `boringssl` sources became a submodule
(`jni/third_party/boringssl`, with a small `jni/openssl/` shim left behind), `jni/td` was added for
tdlib, and `jni/prebuild` grew a **`build_all.sh` pipeline** that builds boringssl, openh264, opus,
libvpx, dav1d, ffmpeg, tlottie and tdlib into `prebuild/lib/<abi>/`. Gradle drives it through a new
`:TMessagesProj:buildNativeDeps` task, and `build_all.sh` starts with `rm -rf "$LIB_DIR"` — there is
**no per-library skip**, so the committed `.a` files are wiped and everything is rebuilt.

- **Three new submodules must be initialised** — `TMessagesProj_Modules/media` (without it gradle
  cannot even evaluate `settings.gradle`), `TMessagesProj/jni/td` and
  `TMessagesProj/jni/third_party/boringssl`. Cloning them from the local modern checkout needs
  `-c protocol.file.allow=always` (git blocks the file transport for submodules by default).
- **`build_tdlib.sh` needs `gperf` on the host** — TDLib generates its mime-type perfect hash with
  it, and without it CMake fails with `GPERF_EXECUTABLE`. The F-Droid recipe already installs it
  (`sudo: apt-get install -y … gperf …`), so this bites only local builds.

### F-Droid recipe — classic needs the same MR the modern app got for 12.10.4

`AutoUpdateMode: Version` clones the *previous* build entry, so the auto-generated entry for the next
classic tag would inherit the pre-12.10.4 paths and fail. Three changes, all mirroring
`org.forkgram.messenger`'s 12.10.4 entry:

- `rm:` add `TMessagesProj_Modules/media/gradle/wrapper/gradle-wrapper.jar` (the new media submodule
  ships a prebuilt wrapper jar).
- `scanignore:` `TMessagesProj/src/main/assets/emoji/metadata.bin` → **`TMessagesProj/emoji/metadata.bin`**
  (the file moved).
- `scanignore:` add `TMessagesProj_Modules/media/publish.gradle` and
  `TMessagesProj_Modules/media/libraries/{cast,datasource_cronet,exoplayer_ima}/build.gradle` plus
  `TMessagesProj_Modules/media/libraries/test_data`.

`sudo:` already lists `gperf`, so it needs no change.

### Audit notes

- **`[classic] #` marker count 676 → 676, nothing lost.** The only per-file movement is
  `FolderIcons.java` → `FolderIcons.kt` (2 markers each).
- **One parity gap the pins created, closed by hand:** 12.10.4's settings lock ships in classic (its
  row lives in the un-pinned `ForkSettingsActivity`), but its *settings-search* entry lived in the
  pinned `ProfileActivity` and so did not come across — re-added as `SearchResult(1059)`.
- **Pre-existing, not from this bump:** `ProfileActivity` uses `SearchResult(1057)` twice
  (`DisableUnifiedPush` from the base, `ShowBottomTabs` from classic's #61). Present pre-bump too.
