...
## Modern `dev` re-base — no new release, one fork commit (the `/classic-bump` run, 2026-08-06)

Not a version bump at all: the newest tag in the modern repo is still `12.9.2.0`, so this run only moved
classic from the **tag commit** `[Release] 12.9.2.` (`a2defcfde`) onto the current **`dev` tip**
`2b2c5260b` ("Bound no-frame decode path so dead decoder cannot pin core."). The upstream snapshot did
**not** move, so as in 12.9.1 there was **no new redesign to undo** — the pins had nothing to revert.

Measured the delta by *subject sets*, never by commit count (`dev` is force-pushed): both sides carry
**101 commits** after their merge-base `777b9dc5ef` ("Monet themes PR"), and the sets differ by exactly
two entries — `dev` gained the decode-path commit, and it lacks `[Release] 12.9.2.`, which is an **empty
marker commit** (0 files), so dropping it costs nothing.

The other 16 changed files are *not* new commits: the force-push **amended** several existing fork
commits (the UnifiedPush PR, "hidden accounts and stealth mode", "WebSocket transport", "satellite
networks"), which is why a subject-identical history still produces a real content delta —
`AppStartReceiver`, `PushListenerController`, `SharedConfig`, `UnifiedPushService`, `Dialogs.kt`,
`ForkSettingsActivity`, `NotificationsSettingsActivity`, `ProfileActivity` and strings in 10 locales.

`AnimatedFileDrawable`'s new decode-path guard merged **silently**: classic already carried the
byte-identical patch, so it does not even appear in the pre/post-rebase diff.

### The one conflict and the one reshim

- **Conflict: pinned `ProfileActivity`, on the pipeline commit.** The settings-search array collided —
  HEAD offered the base's 62-line block (with the `f.presentFragment(…)` style), classic's side was the
  3-line 12.1.1 form of the 402/403/404 help entries. Resolved `--theirs` per the DESIGN_FROZEN rule; the
  fork search block is re-added later by the Phase-2 commit in classic's own `presentFragment(…)` style,
  and it applied cleanly afterwards.
- **Reshim (1): the UnifiedPush setting was split in two.** `ForkSettingsActivity.ID_DISABLE_UNIFIED_PUSH`
  became `ID_UNIFIED_PUSH` (now the distributor/gateway screen), while the on/off switch moved to
  `NotificationsSettingsActivity#disableUnifiedPushRow`, reachable via a new chainable
  `highlightUnifiedPush()`. `ForkSettingsActivity` and `NotificationsSettingsActivity` are un-pinned and
  rode the new base in, but the **pinned** `ProfileActivity` is the *reader* of both — the classic
  **dead-setting** shape. Ported the base's hunk into the pinned file in classic's call style: entry 1051
  retargeted to `ID_UNIFIED_PUSH`/`R.string.UnifiedPush`, plus the new entry 1057 pointing at the
  notifications row (the pinned file already had the 6-arg `SearchResult(id, title, row, section, icon,
  open)` constructor and already used `presentFragment(new NotificationsSettingsActivity())` elsewhere).

Library compiled **0 errors** after that single fix. Contamination audit (pre-rebase classic → rebased
classic) came back to exactly the 17 files the base changed — no pinned file drifted.

## Modern 12.9.7 bump — a real upstream minor (12.10), but no new chrome redesign (the `/classic-bump` run, 2026-08-23)

Base move: the `dev` tip `2b2c5260b` ("Bound no-frame decode path so dead decoder cannot pin core.", the
commit the 2026-08-06 re-base landed on) → `[Release] 12.9.7.` (`3367cea91`, tag `12.9.7.0`). Unlike the
last three runs the **upstream snapshot jumped a whole minor**: `b7561f0c6 update to 12.9.2 (6991)` →
`4e1a61eca update to 12.10.0 (7031)` — 326 java files, +18757/−19022. Version: `12.10.0` / code `7032` =
base (7031) + 1.

### There was no redesign to undo — measure before you hunt for one

The 12.8 run's headline was reverting "glass mode"; this one has no equivalent. The upstream delta on the
**pinned chrome** is small and behavioural: `ActionBar` +29/−3, `ActionBarLayout` +14/−14, `EmojiView`
+1/−1, and `ShareAlert`/`DrawerLayoutContainer` **unchanged**. Where 12.10 actually changed pixels is the
**message-bubble layer** — new `ui/ActionBar/MessageDrawable` (extracted out of `Theme.MessageDrawable`),
`Components/TornEdge`, `Components/QuoteCollapseButton`, `Components/UnsupportedBlockDrawable`,
`Cells/ChatMessageUnsupportedCell`, and `ChatMessageCell` itself (+315/−79). **Classic keeps
`ChatMessageCell` un-pinned** (upstream-wholesale, for poll v2), so it *adopts* that layer rather than
reverting it. Device check confirmed the bubbles still read as classic. The other new surface is a
feature, not a redesign: `Gifts/GiftMessageBottomSheet` + `GiftMessageDrawable`/`GiftMessageView`/
`StarGiftUniqueActionView`, plus more `iv/Rich*` files.

So the whole run was step §4 — re-shimming the 12.1.1 pins against a drifted core.

### The reshims

1. **`SharedConfig.noStatusBar` deleted upstream** → re-added as a `public static final boolean = true`.
   The pinned `ActionBar`, `ActionBarLayout` and `ProfileActivity` still consult it.
2. **Classes relocated out of `androidx.recyclerview.widget` into `org.telegram.ui.recyclerview`** —
   `LinearSmoothScrollerCustom` (pinned `EmojiView`, `ProfileActivity`) and `ChatListItemAnimator`
   (pinned `ChatActivityEnterView`). Import-only, but it must flow into the pins or they don't compile.
3. **`Theme.MessageDrawable` → top-level `ui/ActionBar/MessageDrawable`** — `ActionBarLayout`'s two
   crossfade fields and the two `INavigationLayout` overrides retyped.
4. **Send parameters consolidated** (the big one). 12.10 replaced the `quickReplyShortcut` +
   ...
   implementation **minus its `MODE_WELCOME_MESSAGES` branch** (the pinned chat screen never hosts that
   mode), and rewrote every pinned call site in `ChatActivity`/`ChatActivityEnterView` to pass it —
   including `MediaController.startRecording`/`prepareResumedRecording` and
   `SendMessagesHelper.SendMessageParams.sendMessageChatArguments`.
5. **`prepareSendingPhoto`/`prepareSendingVideo` grew a `scheduleRepeatPeriod`** inserted *between*
   `scheduleDate` and `forceDocument`/`mode`. Padded with `0` at the six pinned call sites — safe here
   because classic ships no repeat-schedule UI, but this is exactly the
   [[feedback_widened_signature_padded_call]] shape: check what the literal means before padding.
6. **`MODE_WELCOME_MESSAGES = 9`** added to the pinned `ChatActivity`. Classic never enters the mode; the
   constant exists because un-pinned `MessagesController`/`MessagesStorage`/`MessageObject`/
   `ChatMessageCell`/`ChatAvatarContainer` read it off `ChatActivity`.
7. **TL keyboard schema rewritten**: `TLRPC.KeyboardButton` → `TL_keyboard.KeyboardButtonProto`, with the
   button *kind* moved from the subclass into a `type` field, queried via
   `TLKeyboardHelper.getType/isType`. `ChatActivityEnterView.didPressedBotButton` was ported from the
   12.10 upstream implementation — this is behaviour, not layout, so the design pin does not apply. New
   TL relocation to remember alongside 12.8's `TLRPC.PageBlock`→`TL_iv`.
8. **`BaseFragment.createView` demoted to `protected`** behind a new `performCreateView(Context)` trace
   wrapper; the pinned file takes the new base's form (needs the `BuildConfig` import).
9. **Two `GiftMessageBottomSheet` hosting helpers** on the pinned `ChatActivityEnterView`:
   `setCustomWindowView(View)` (anchor popups to the sheet, not the window) and
   `static disableNewLines(EditText)` for its single-line comment field.

### Audit notes

- **The `[classic] #` marker count fell 641 → 640 and nothing was lost.** The missing one is
  `SharedConfig`'s `#109` UnifiedPush-gateway comment: the fork **upstreamed classic's fix**, so the
  `unifiedPushGatewayMigrated` logic now ships in the base with the tag stripped and the comment
  reworded. Same shape as 12.9's `#79`. Diff the *logic*, not the marker, before calling a rebase lossy.
- **Pinned-file contamination audit** (pre-bump classic → rebased+reshimmed) came back to exactly the
  reshims above; `ChatAttachAlert`, `ChatAttachAlertPollLayout`, `ShareAlert`, `ActionBar` and
  `DrawerLayoutContainer` are byte-identical to pre-bump.
- Boxed-`Boolean` scan (12.8 lesson): **no new** `public Boolean` fields in 12.10 vs 12.9.2 — clean.
- **Native**: `jni/**` reads 816 files / −298138 lines because upstream slimmed the vendored
  `opus`/`mozjpeg`/`rlottie`/`exoplayer` sources. `TMessagesProj/jni/dav1d` (20M) and
  `TMessagesProj/jni/libvpx` (39M) are **untracked build-time leftovers** in the tree — expected, same
  category as the `jni/libvpx` exception in the skill's §0 precondition.
- Build: library `compileDebugJavaWithJavac` **0 errors**; `:TMessagesProj_App:assembleAfatDebug
  -PF_DROID=1 -Parmeabi-v7a` **BUILD SUCCESSFUL in 12m09s** (native cache warm despite the jni churn).
- Device-verified on RMX3581 (`12.10.0.0`, code `703209`): solid continuous chat action bar (back ·
  avatar · name/subtitle · menu — no capsules), classic dialogs with hamburger + flat folder tabs, legacy
  profile with the flat media tab strip, classic attach sheet, Emoji/GIFs/Stickers panel scrolling
  without the #74 vanish, pinned-message bar flush under the action bar.

## Modern 12.10.3 bump — no new redesign, zero reshims (the `/classic-bump` run, 2026-09-10)

Base move `[Release] 12.9.7.` (`3367cea91`) → `[F-Droid] 12.10.3.` (`d37ded1ba`, fetched from the modern
repo as tag `12.10.3.0`; the tag is published on origin). Upstream snapshot moved to **12.10.1 (7038)** —
a patch-level step, so there is **no new chrome redesign to undo**. Version: `12.10.3` / code `7041` =
base (7040) + 1, monotonic over classic's last release (7034).

**The reshim climb did not happen.** `:TMessagesProj:compileDebugJavaWithJavac` came back **0 errors on
the first attempt** — the first bump in this series to need no shim at all. The glass API that 12.8
introduced still has live callers (`ChatAvatarContainer` → `checkAvatarContainerWidth`, the two community
activities → `setupGlass`), but the no-op shims added in earlier runs already cover them, and the new base
added no further redesign surface to the pinned classes.

### Conflict resolutions (5 commits conflicted, 11 files)

- **Pinned, take classic** — `ActionBar` (5 hunks: `setupGlass`/`glassDrawable*`/`searchFactor`/
  `checkAvatarContainerWidth` bodies), `ChatActivity` (4 hunks: `setupGlass` wiring, the input-island
  containers, `topPanelLayout`, `didPressStreamingStop`), `ChatActivityEnterView` (send-button slide +
  `sendButtonBlockedByTypingView`), `DialogCell`.
- **`DialogCell` is the one that looked like a genuine feature and is not.** The base's hunk is
  community-aware swipe-mute (`isDialogCommunity() ? isCommunityMuted(...) : dialogMuted`). Taking the
  base side does **not** compile: classic's pinned `DialogCell` carries none of the community fields
  (12 occurrences in the base, 0 in the pinned file). Verify the symbols exist before "rescuing" a hunk.
- **`prebuild_fdroid.sh` — take the BASE, not classic.** Both sides carry the same `#113` Rust-toolchain
  fix, but the jni restructure renamed the script: classic's `./tlottie_lib/build.sh` is now
  `./prebuild/build_tlottie.sh` (and the `git checkout --` guard moved from `tlottie_lib` to `prebuild`).
  Keeping the classic side would have re-armed `#113` — the guard exits 1 and the F-Droid build dies,
  exactly the 12.10.1 bad release. Took the base's paths and re-added the `# [classic] #113:` tag.
- **`NotificationsService` — base's code, classic's hunk re-applied.** The base replaced the inline
  foreground block with `moveToForeground()`, which is upstream's fix for *"Fixed crash when the push
  service was stopped before going foreground"* (`RemoteServiceException: Context.startForegroundService()
  did not then call Service.startForeground()`) — worth having, it fires on low-RAM devices. But its
  `getNotification()` is back on the hardcoded `R.drawable.notification`, so `#54` (icon follows the
  selected app icon) had to be re-applied there. Final file = base + the `LauncherIconController` import
  + the `#54` line + two rebranded log tags.
- **`ApplicationLoader`** — base's `stopPushService()` helper, classic's rebranded log tag.
  **`FilterTabsView`** — base's declaration form, classic's `#79` marker re-attached (the
  `defaultTab != null` guard the marker refers to auto-merged either side of it).
- **`.github/workflows/tandroid.yml`** — modify/delete; classic deletes the modern CI workflow (it ships
  only via F-Droid, keeping `FUNDING.yml` + `docker-builder.yml`). Deletion kept.

### Native

`jni/**` reads 383 files / −241719 lines: upstream moved the vendored `third_party` sources to
**submodules**, adding `third_party/openh264` and `third_party/libyuv`, and renamed `jni/tlottie_lib` →
`jni/prebuild`. All submodules were already checked out, so the native cache stayed warm
(`assembleAfatDebug` **BUILD SUCCESSFUL in 4m56s**). `jni/dav1d`, `jni/libvpx` and now `jni/tlottie_lib`
sit in the tree as **untracked leftovers of the old layout** — same category as the `jni/libvpx`
exception in the skill's §0 precondition; nothing reads them any more (`prepare.py:420` points at
`./prebuild/build_tlottie.sh`).

### Audit notes

- **`[classic] #` marker count fell 647 → 643 and nothing was lost.** All four lines are `#115` in
  `OfflineLanguageDetector` / `TranslateController`: the fork **took classic's fix upstream** (it now
  ships inside `[TF][KILL] Google Translate without MLKit`) and its repo bans comments, so the tags are
  stripped. The logic is verified present in the base — `containsWord`/`isWordCharacter`/
  `NON_SPACING_MARK`, the Nepali and Marathi markers, and a live `checkLanguage`. The classic `#115`
  commit was therefore **dropped during the rebase** (`git rebase --skip`) rather than re-applied.
  Third time this shape appears (12.9's `#79`, 12.10's `#109`): diff the *logic*, not the marker.
- The uncommitted `#97` work found in the tree was committed **before** the rebase so it would ride the
  stack rather than be stranded in a stash against a pinned file the rebase rewrites.
- Device-verified on RMX3581 (`12.10.3.0`, code `7041`): solid continuous chat action bar (back · avatar ·
  name/subtitle · menu — no capsules), classic dialogs with hamburger + flat folder tabs, pinned-message
  bar flush under the action bar, no crashes across dialogs/chat/drawer/profile.
- **Not caused by this bump:** the fork's optional bottom tab bar (Chats/Contacts/Settings/Profile) draws
  as a floating rounded capsule. `MainTabsLayout.java` is byte-identical pre- and post-bump and the base
  never touched it, so this is pre-existing classic-fidelity debt in the same family as `#59`/`#61`.
