...
The single source of truth for the launcher icon set: edit the SVG (or the size
constants below), re-run, rebuild.

What it produces:
  * icon_01_background_sa          - background only (the blue gradient square).
  * icon_01_foreground[_round]     - foreground only (fork on transparent), used as
                                     the adaptive <foreground> over a background
                                     carrying icon_background_clip, a white matte
                                     with a ~0.53R transparent disc. FORK_FG_HOME
                                     stays small so the fork sits inside that disc
                                     instead of spilling past the ring.
  * icon_01_foreground_sa          - foreground for the _sa / _adaptive XML, which
                                     sits over the full-bleed gradient with no clip
                                     matte and doubles as the in-app icon-picker
                                     preview, so it can be larger.
  * icon_01_launcher[_round]       - legacy composite PNG: gradient circle + fork +
                                     white outline ring.
  * icon_01_launcher_sa/_adaptive  - the same composite without the ring.
  * icon_fork.xml                  - monochrome themed-icon vector, built from the
                                     SVG fork polygons (exact, no tracing).
  * splash_fork_320.xml            - splash vector: gradient disc + the three
                                     faceted fork gradients.
...
ImageMagick (built against librsvg) rasterises the gradient and the fork to
high-res masters, Pillow does the compositing, masks and rings, and the two
Android vectors are emitted straight from the source polygons.

Requirements: `magick` with the RSVG delegate, and Pillow.
...
# 2-stop linear gradients (userSpaceOnUse), keyed by id. Used to emit the
# faceted splash vector; the gradient coords live in the same pre-transform
# space as the polygon points, so they ride the group transforms identically.
GRAD = {}
for gm in re.finditer(
        r'<linearGradient id="(\w+)"[^>]*?x1="([-\d.]+)" y1="([-\d.]+)" '
        r'x2="([-\d.]+)" y2="([-\d.]+)">.*?stop-color="(#[0-9a-fA-F]+)"'
        r'.*?stop-color="(#[0-9a-fA-F]+)"', svg, re.S):
    GRAD[gm.group(1)] = dict(x1=gm.group(2), y1=gm.group(3), x2=gm.group(4),
                             y2=gm.group(5), c0=gm.group(6), c1=gm.group(7))
...
# splash_fork_320.xml - faceted fork (the launcher icon's three gradient facets)
# on a gradient disc (viewport 320, disc diameter 200). Static; the Android-12
# splash screen masks/sizes it. The soft drop-shadow of the icon is intentionally
# dropped (vector drawables have no blur) - the facets carry the identity.
def grad_path(n):
    """A single faceted polygon as a <path> with an inline linear gradient."""
    g = GRAD[n]
    p = POLY[n]
    d = "M%s" % ",".join(f"{x:g} {y:g}" for x, y in p[:1])
    d += "".join(f"L{x:g} {y:g}" for x, y in p[1:]) + "Z"
    return (f'<path android:pathData="{d}">'
            ...
            f'<gradient android:type="linear" android:startX="{g["x1"]}" '
            f'android:startY="{g["y1"]}" android:endX="{g["x2"]}" '
            f'android:endY="{g["y2"]}" android:startColor="{g["c0"]}" '
            f'android:endColor="{g["c1"]}"/></aapt:attr></path>')


s, tx, ty = g0_for(320, FORK_SPLASH * 200)
DISC_D = ("M160,60 C133.49,60 108.04,70.54 89.29,89.29 C70.54,108.04 60,133.49 "
          "60,160 C60,186.51 70.54,211.96 89.29,230.71 C108.04,249.46 133.49,260 "
          "160,260 C186.51,260 211.96,249.46 230.71,230.71 C249.46,211.96 260,186.51 "
          "260,160 C260,133.49 249.46,108.04 230.71,89.29 C211.96,70.54 186.51,60 160,60 Z")
bgg = GRAD["bg"]
facets = "".join(grad_path(n) for n in ("paperMid", "paperLight", "paperDark"))
(OUT / "drawable" / "splash_fork_320.xml").write_text(
    ...
    f'<path android:pathData="{DISC_D}">'
    '<aapt:attr name="android:fillColor">'
    f'<gradient android:type="linear" android:startX="160" android:startY="60" '
    f'android:endX="160" android:endY="260" android:startColor="{bgg["c0"]}" '
    f'android:endColor="{bgg["c1"]}"/></aapt:attr></path>'
    f'<group android:translateX="{tx:.3f}" android:translateY="{ty:.3f}" '
    f'android:scaleX="{s:.5f}" android:scaleY="{s:.5f}">'
    '<group android:pivotX="256" android:pivotY="256" android:rotation="-40" '
    'android:scaleX="1.14" android:scaleY="1.34">'
    f'{facets}</group></group></vector>\n')
