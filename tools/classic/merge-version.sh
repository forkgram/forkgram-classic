#!/usr/bin/env bash
# Stage an upstream version for review. Mutates nothing: it categorizes the
# diff via tools/classic/categorize-diff.sh, then prints the `git checkout`
# for the ALWAYS_MERGE delta, a reminder to re-pin DESIGN_FROZEN paths from
# baseline, and the `unknown` list that needs manual classification.
#
# Usage:
