...
public class ContactAddActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate, ImageUpdater.ImageUpdaterDelegate {
    ...
    // [classic] #16: notes kept (wired into ProfileActivity edit-notes path) but rendered as a classic underlined field.
    ...
    public View createView(Context context) {
            ...
            public void onItemClick(int id) {
                if (id == -1) {
                    ...
                    if (firstNameField.getText().length() != 0) {
                        ...
                        user.contact = true;
                        // [classic] #16: build the note from the classic underlined field.
                        final TLRPC.TL_textWithEntities note = getNoteTextWithEntities();
                        getMessagesController().putUser(user, false);
                        // [classic] #56: pass the note into the contacts.addContact request (flag 2) so it is
                        // saved on the server, like modern. The old 2-arg overload dropped the note, so it was
                        // only written to local MessagesStorage and got wiped on the next userFull reload from
                        // the server (and was skipped entirely while adding a contact, when userInfo is null) ->
                        // "edited note won't save". addContact(user, note, ...) both adds and updates the note.
                        getContactsController().addContact(user, note, checkBoxCell != null && checkBoxCell.isChecked());
        ...
        // [classic] #16: restore the pre-12.0 plain white scroll layout (ref 11.9.5.0); the
        // redesign used a gray UniversalRecyclerView with card insets, EditTextCell fields and
        // extra suggest-birthday / in-form delete rows.
        ...
        // [classic] #16: notes field kept (ProfileActivity edit-notes path), styled as the classic underlined input.
        noteField = new EditTextBoldCursor(context) {
            @Override
            protected Theme.ResourcesProvider getResourcesProvider() {
                return resourcesProvider;
            }
        };
        noteField.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 18);
        noteField.setHintTextColor(Theme.getColor(Theme.key_windowBackgroundWhiteHintText, resourcesProvider));
        noteField.setTextColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlackText, resourcesProvider));
        noteField.setLinkTextColor(getThemedColor(Theme.key_chat_messageLinkIn));
        noteField.setBackgroundDrawable(null);
        noteField.setLineColors(getThemedColor(Theme.key_windowBackgroundWhiteInputField), getThemedColor(Theme.key_windowBackgroundWhiteInputFieldActivated), getThemedColor(Theme.key_text_RedRegular));
        noteField.setMaxLines(4);
        // [classic] #16: align the "Add notes" placeholder with the name fields. The name fields sit in a
        // fixed dp(36) box, leaving a comfortable gap between their single line of text and the underline.
        // The note field is multi-line + WRAP_CONTENT (so it can still grow to 4 lines), which collapses
        // the box onto the text and drops the underline right under the placeholder (measured 9px gap vs
        // the name fields' ~25px). setMinHeight is ignored on this multi-line field, so instead extend the
        // bottom padding to recreate the name fields' below-text gap while keeping WRAP_CONTENT growth
        // (extra lines add height above this padding). setLineColors() above already applied the classic
        // search_dark insets; preserve left/top/right and only grow the bottom. TOP gravity anchors the
        // first line to the top of the box like the name fields' default-TOP single line.
        noteField.setPadding(noteField.getPaddingLeft(), noteField.getPaddingTop(), noteField.getPaddingRight(), noteField.getPaddingBottom() + AndroidUtilities.dp(8));
        noteField.setGravity(Gravity.TOP | (LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT));
        noteField.setInputType(InputType.TYPE_TEXT_FLAG_CAP_SENTENCES | InputType.TYPE_TEXT_FLAG_AUTO_CORRECT | InputType.TYPE_TEXT_FLAG_MULTI_LINE);
        noteField.setImeOptions(EditorInfo.IME_ACTION_DONE);
        noteField.setHint(LocaleController.getString(R.string.AddNotes));
        noteField.setCursorColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlackText, resourcesProvider));
        noteField.setCursorSize(AndroidUtilities.dp(20));
        noteField.setCursorWidth(1.5f);
        linearLayout.addView(noteField, LayoutHelper.createLinear(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, 24, 16, 24, 0));
        noteField.setOnEditorActionListener((textView, i, keyEvent) -> {
        ...
        final TLRPC.UserFull userFullForNote = getMessagesController().getUserFull(user_id);
        if (userFullForNote != null && userFullForNote.note != null) {
            noteField.setText(MessageObject.formatTextWithEntities(userFullForNote.note, false));
        }
...
//            getMessagesController().loadDialogPhotos(user_id, 2, 0, true, getClassGuid(), null);
        ...
        if (focusNotes) {
            AndroidUtilities.runOnUIThread(() -> {
                noteField.requestFocus();
                AndroidUtilities.showKeyboard(noteField);
            }, 200);
        }
    ...
    // [classic] #16: replicate EditTextCell.getTextWithEntities for the classic underlined notes field.
    private TLRPC.TL_textWithEntities getNoteTextWithEntities() {
        final TLRPC.TL_textWithEntities text = new TLRPC.TL_textWithEntities();
        final CharSequence[] message = new CharSequence[]{ noteField.getText() };
        text.entities = MediaDataController.getInstance(currentAccount).getEntities(message, true);
