...
public class UserCell extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, Theme.Colorable {
    ...
    private boolean adminTagIsSelf; // [classic] #89, widened by #97
    public void setAdminRole(String role, boolean isAdmin, boolean isOwner, boolean canAddTag, View.OnClickListener onClick) {
        ...
        // [classic] #89: the tag button on your own row has no other action competing with it, so the
        // whole cell may act as the button (see isInsideAdminTag()).
        // [classic] #97: that holds just as much once the tag exists — editing it must not be harder
        // to hit than adding it was. canAddTag is only ever passed for your own row (every call site
        // computes it as isUserSelf() && canManageMyTag()), so it alone marks the inert row.
        adminTagIsSelf = canAddTag;
    ...
    private boolean isInsideAdminTag(float x, float y) {
        ...
        if (adminTagIsSelf) {
            ...
            // spam clicking" while adding one did not.
