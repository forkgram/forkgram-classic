...
public class UserCell extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, Theme.Colorable {
    ...
    private boolean adminTagIsAddSelf; // [classic] #89
    public void setAdminRole(String role, boolean isAdmin, boolean isOwner, boolean canAddTag, View.OnClickListener onClick) {
        ...
        // [classic] #89: "Add Tag" is only ever offered on your own row, and that row has no other
        // action of its own, so the whole cell may act as the button (see isInsideAdminTag()).
        adminTagIsAddSelf = canAddTag && !isAdmin && !isOwner && TextUtils.isEmpty(role);
