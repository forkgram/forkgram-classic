...
public class UserCell extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, Theme.Colorable {
    ...
    public void setAdminRole(String role, boolean isAdmin, boolean isOwner, boolean canAddTag, View.OnClickListener onClick) {
        ...
        // [classic] #97: the press used to be reset right here, unconditionally (#89: "a recycled
        // cell must not keep the pressed state of its previous binding"). But the adapters re-run
        // this binding for the SAME member on every updateInterfaces(UPDATE_MASK_STATUS), which
        // MessagesController.putUsers() posts for nearly every server response — a busy account
        // gets a steady stream of them. One landing between the finger's DOWN and UP killed the
        // tap ("editing a tag needs spam clicking"), while a tap on the label itself survived,
        // because the TextView's own pressed state is never touched here. The reset now lives in
        // setData(), keyed on the bound member actually changing.
    ...
    public void setData(Object object, TLRPC.EncryptedChat ec, CharSequence name, CharSequence status, int resId, boolean divider) {
        // [classic] #97: the tag press only dies when the cell is handed to someone else (recycling);
        // a rebind of the same member — every status update does one — keeps it.
        if (!isSameEntity(currentObject, object)) {
            resetAdminPress();
        }
    ...
    // [classic] #97: see setAdminRole() — called when the cell changes hands, not on every rebind.
    private void resetAdminPress() {
        adminPressed = false;
        adminLongPressed = false;
        if (adminTextView != null) {
            adminTextView.setPressed(false);
    ...
    private static boolean isSameEntity(Object a, Object b) { // [classic] #97
        if (a == b) {
            return true;
        }
