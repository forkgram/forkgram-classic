...
//            imageViewParams.height = mono ? dp(28) : dp(36);
    ...
    // input grows. [classic] #102: this was a no-op on the assumption that upstream
    // adjusts the tabs internally — it does not, it feeds the same value in from
    // ChatActivity.updateBotforumTabsBottomMargin(), part of the 12.7 inset pipeline
    // classic dropped. Unwired, the side column ran the full content height, so its last
    // row sat behind the input bar and the list would not scroll it into view.
