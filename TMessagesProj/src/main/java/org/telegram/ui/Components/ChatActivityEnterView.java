...
public class ChatActivityEnterView extends BlurredFrameLayout implements NotificationCenter.NotificationCenterDelegate, SizeNotifierFrameLayout.SizeNotifierFrameLayoutDelegate, StickersAlert.StickersAlertDelegate, SuggestEmojiView.AnchorViewDelegate {
    ...
    public interface ChatActivityEnterViewDelegate {
        ...
        // forkgram-classic: upstream sig grew a `scheduleRepeatPeriod` slot
        // between scheduleDate and payStars. Pinned call sites in this file
        // pass 0; ChatActivity's override forwards it through.
        ...
        // forkgram-classic: upstream sig grew a `scheduleRepeatPeriod` slot
        // between scheduleDate and ttl. Inserted here so DialogsActivity's
        // override matches; pinned call sites pass 0.
        ...
        // forkgram-classic: upstream PeerStoriesView's delegate exposes a
        // story-specific "default send-as" peer for live comments. Classic
        // chats don't use it — provide nullable defaults.
        ...
        // forkgram-classic: upstream PeerStoriesView routes audio sends
        // through the delegate (live-comment audio path). Default impl
        // forwards to SendMessagesHelper directly; ignore the optional flags.
        default void sendAudio(java.util.ArrayList<MessageObject> audios, CharSequence caption, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long effectId, boolean invertMedia, long payStars) {
        }
    }
    ...
    @Nullable
    // forkgram-classic: PeerStoriesView reaches in to animate the caption
    // limit indicator directly; relax to public.
    ...
    // forkgram-classic: PeerStoriesView reads this to gate the suggestion
    // button visibility; relax to public.
    ...
    // forkgram-classic: upstream DialogsActivity reaches in to tweak the
    // padding of the text field container directly; relax to public.
    public FrameLayout textFieldContainer;
    // forkgram-classic: PeerStoriesView animates the send button container's
    // alpha during story composition; relax to public.
...
//        ScaleStateListAnimator.apply(sendButton);
    ...
    private void createGiftButton() {
        ...
        giftButton.setOnClickListener(v -> {
            ...
            // forkgram-classic: BoostRepository.loadGiftOptions was removed
            // upstream; gift loading moved to a different API surface. Skip the
            // load and open GiftSheet with empty options for now.
            progressDialog.dismiss();
            new GiftSheet(getContext(), currentAccount, user.id, java.util.Collections.emptyList(), null).setBirthday(birthday).show();
            final int reqId = 0;
        });
    }
                    ...
                    AndroidUtilities.runOnUIThread(() -> {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                ...
                                AndroidUtilities.setLightNavigationBar(d.getWindow().getDecorView(), brightness >= 0.721f);
                            }
                        }
                        float offX = 0, offY = 0;
...
//                }
    ...
    private void openWebViewMenu() {
        ...
        Runnable onRequestWebView = () -> {
            ...
            if (botMenuWebViewUrl != null && (botMenuWebViewUrl.contains("/start?startapp=") || botMenuWebViewUrl.contains("?startapp=") || botMenuWebViewUrl.contains("&startapp="))) {
                ...
                if (view instanceof BotCommandsMenuView.BotCommandView) {
                    ...
                    if (isInScheduleMode()) {
                        AlertsCreator.createScheduleDatePickerDialog(parentActivity, dialog_id, (notify, scheduleDate, scheduleRepeatPeriod) -> {
                            final SendMessagesHelper.SendMessageParams params = SendMessagesHelper.SendMessageParams.of(command, dialog_id, replyingMessageObject, getThreadMessage(), null, false, null, null, null, notify, scheduleDate, 0, null, false);
...
//        });
            ...
            try {
                ...
                final InputConnectionCompat.OnCommitContentListener callback = (inputContentInfo, flags, opts) -> {
                    ...
                    if (inputContentInfo.getDescription().hasMimeType("image/gif") || SendMessagesHelper.shouldSendWebPAsSticker(null, inputContentInfo.getContentUri())) {
                        if (isInScheduleMode()) {
                            AlertsCreator.createScheduleDatePickerDialog(parentActivity, parentFragment.getDialogId(), (notify, scheduleDate, scheduleRepeatPeriod) -> send(inputContentInfo, notify, scheduleDate), resourcesProvider);
                        } else {
    ...
    // forkgram-classic: upstream DialogsActivity uses the 2-arg overload
    // (no separate line view). Forward with a null lineView.
    ...
    // forkgram-classic: upstream DialogsActivity attaches an inset controller
    // for the Material 3 dynamic island layout. The classic enter view
    // doesn't know about that — accept and drop.
    public void setInAppInsetsController(org.telegram.ui.Components.inset.WindowInsetsInAppController inAppInsetsController) {
    }

    // forkgram-classic: upstream DialogsActivity points the emoji popup at
    // an explicit parent (chatInputInAppContainer). Classic emoji view
    // mounts itself under the action bar — ignore the redirection request.
    public void setViewParentForEmoji(android.view.ViewGroup viewParent) {
    }

    // forkgram-classic: upstream subclasses override this hook to react to
    // dynamic-island height changes. Classic enter view has nothing to do
    // here; provide an empty implementation so DialogsActivity's override
    // resolves.
    protected void onChangedIslandTotalHeight(float newTotalHeight) {
    }

    // forkgram-classic: upstream PeerStoriesView reads/writes the message
    // composer's vertical animation offset directly. The pinned class
    // already stores `animatedTop` (line 617) but only assigns it
    // internally; expose the setter and the current top-view enter
    // progress for the stories overlay.
    protected void setAnimatedTop(int top) {
        animatedTop = top;
        invalidate();
    }
    ...
    // forkgram-classic: upstream PeerStoriesView reads the current message
    // text with formatting entities for the live-comment send path.
    // Classic flow doesn't need entities here — return null so callers
    // fall back to plain text handling.
    public org.telegram.tgnet.TLRPC.TL_textWithEntities getTextWithEntities() {
        return null;
    }

    // forkgram-classic: upstream PeerStoriesView toggles a "live comment"
    // appearance on the enter view. Classic enter view doesn't have live
    // comments — accept and drop.
    public void setLiveComment(boolean liveComment, boolean disabledPaid) {
    }

    // forkgram-classic: upstream PeerStoriesView wires a dedicated side
    // controls layout into the enter view's attach button. Classic flow
    // uses the standard attach popup — accept and ignore.
    public void setSideButtonsForAttach(android.view.View sideControls) {
    }

    // forkgram-classic: upstream PeerStoriesView overrides this to report
    // whether live-comment paid features are disabled for the viewer.
    // Classic enter view has no paid live comments — always free.
    public boolean areLiveCommentsFree() {
        return true;
    }

    // forkgram-classic: upstream PeerStoriesView wires a send-button long-press
    // for the "send as" picker. The classic enter view owns that behaviour
    // internally — accept and drop.
    public void setOnSendButtonLongClick(android.view.View.OnLongClickListener listener) {
    }

    // forkgram-classic: upstream PeerStoriesView reads the sender select
    // view from the enter view. Classic enter view holds it as a private
    // field; expose via this accessor.
...
//        }
    ...
    // forkgram-classic: PeerStoriesView formats hint text using these spans.
...
//        }
    ...
    // forkgram-classic: upstream PeerStoriesView overrides the 5-arg form
    // (with scheduleRepeatPeriod). Classic flow ignores the repeat period;
    ...
    public boolean processSendingText(CharSequence text, boolean notify, int scheduleDate, long payStars) {
        ...
        if (text.length() != 0) {
            ...
            do {
                ...
                SendMessagesHelper.SendMessageParams params = SendMessagesHelper.SendMessageParams.of(message[0].toString(), dialog_id, replyingMessageObject, replyToTopMsg, messageWebPage, messageWebPageSearch, entities, null, null, notify, scheduleDate, 0, sendAnimationData, updateStickersOrder);
                params.quick_reply_shortcut = parentFragment != null ? parentFragment.quickReplyShortcut : null;
                params.quick_reply_shortcut_id = parentFragment != null ? parentFragment.getQuickReplyId() : 0;
                params.effect_id = effectId;
                params.payStars = payStars;
    ...
    // forkgram-classic: PeerStoriesView re-runs the send-button state from
    // its subclass; relax to public.
...
//        return botWebViewMenuContainer != null && botWebViewMenuContainer.onBackPressed();
    ...
    private void createEmojiView() {
        ...
        emojiView.setDelegate(new EmojiView.EmojiViewDelegate() {
            ...
            public void onGifSelected(View view, Object gif, String query, Object parent, boolean notify, int scheduleDate) {
                ...
                if (isInScheduleMode() && scheduleDate == 0) {
                    AlertsCreator.createScheduleDatePickerDialog(parentActivity, parentFragment.getDialogId(), (n, s, rp) -> onGifSelected(view, gif, query, parent, n, s), resourcesProvider);
                } else {
                    if (slowModeTimer > 0 && !isInScheduleMode()) {
                        if (delegate != null) {
                            delegate.onUpdateSlowModeButton(view != null ? view : slowModeButton, true, slowModeButton.getText());
    ...
    public void onStickerSelected(TLRPC.Document sticker, String query, Object parent, MessageObject.SendAnimationData sendAnimationData, boolean clearsInputField, boolean notify, int scheduleDate, int scheduleRepeatPeriod) {
        ...
        if (isInScheduleMode() && scheduleDate == 0) {
            AlertsCreator.createScheduleDatePickerDialog(parentActivity, parentFragment.getDialogId(), (n, s, rp) -> onStickerSelected(sticker, query, parent, sendAnimationData, clearsInputField, n, s, rp), resourcesProvider);
        } else {
            AlertsCreator.ensurePaidMessageConfirmation(currentAccount, dialog_id, 1, stars -> {
    ...
    // forkgram-classic: upstream DialogsActivity invokes the 3-arg form
    // (with waitKeyboardOpen). Classic implementation doesn't gate on the
    // keyboard explicitly — forward to the 2-arg version.
    public boolean hidePopup(boolean byBackButton, boolean forceAnimate, boolean waitKeyboardOpen) {
        return hidePopup(byBackButton, forceAnimate);
    }
    ...
    public boolean closeCreationLinkDialog() {
        return messageEditText != null && messageEditText.closeCreationLinkDialog(true);
    }
...
//                }
            ...
            // [classic] slowmode: ephemeral (bot-only) messages must not re-arm the slow
            // mode countdown - the 11.9.5.0 restore dropped this guard, which MessagesController
            // still has, so every ephemeral message restarted the wait and blocked sending.
...
//            }
    ...
    // insets already negated (left = -dp(10), right = -(dp(10) + side buttons)). The classic body
    // takes one positive padding — 11.9.5.0 always passed dp(10) — and derives the right inset
    // itself (padding + dp(40)), so only the left may be forwarded: folding in the right would
    // pass ~90-130dp and shove the emoji button, attach layout and send button outside the pill
    // drawChild() clips to.
...
//            }
    ...
    public static class SendButton extends View {
        ...
        public void setBlurredBackgroundDrawable(android.graphics.drawable.Drawable d) {
        }

        // forkgram-classic: upstream DialogsActivity calls this to colour the
        // scrim under the button when the send menu pops open. Classic
        // send button paints its background directly; ignore.
