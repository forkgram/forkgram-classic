...
//        return getString(R.string.AudioUnknownTitle);
    ...
    // forkgram-classic: classic restores a flat profile header, so this is a plain list row
    // (musicRow), not an overlay tied to the avatar-expand animation. updatePosition() is
    // never called there, leaving currentHeight at 0 and the draw/touch code gated behind
    // alpha<=0. Static mode renders the pill full size, makes it tappable, and skips the
    // avatar frosted-glass sample, which a row that scrolls with the list would misalign.
